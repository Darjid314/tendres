...
def send_email_alert(new_tenders_list):
    ...
    msg["Subject"] = (
        f"🚀 Alert: {len(new_tenders_list)} New Oman Tenders Matched!"
    )
...
def translate_to_english(text):
    ...
    for attempt in range(2):
        ...
        try:
            ...
            if translated and "500 (server error)" in translated.lower():
                time.sleep(1)
                continue
...
def parse_sequential_dates(full_text):
    ...
    s_start, s_end, p_start, p_end, sub_close, bid_open = [
        "N/A"
    ] * 6
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        print(
            "🚀 System Init: Connecting to Oman Tender Board Cloud Node..."
        )
        ...
        for current_page in range(
            ...
            url = (
                f"{base_url}&pageNo={current_page}"
            )
            ...
            for index, row in enumerate(rows):
                ...
                try:
                    ...
                    if await clickable_icon.count() > 0:
                        ...
                        try:
                            ...
                            if is_real_url(popup_url):

                                tender_url = popup_url
                            ...
                            if (
                                is_real_url(current_url)
                                and "publicDash" not in current_url
                            ):

                                tender_url = current_url
                ...
                english_agency = translate_to_english(
                    agency
                )
...
if __name__ == "__main__":
    asyncio.run(main())    clean_dates = [f"'{d.strip()}" for d in raw_dates if d.strip()]
                    ...
                    try:
                        ...
                        if await clickable_icon.count() > 0:
                            ...
                            if popup_html:
                                ...
                                if gov_ar: 
                                    gov = translate_to_english(gov_ar.group(1).split('\t')[0].strip())
                                    
                                state_ar = re.search(r"(?:الولاية|State|Wilayat)\s*:\s*([^\n\r\t\d:]+)", raw_text, re.IGNORECASE)
                                if state_ar: 
                                    state = translate_to_english(state_ar.group(1).split('\t')[0].strip())
                                    
                                bg_ar = re.search(r"(?:الضمان|Guarantee|Bank guarantee value)\s*:\s*([^:\n\r]+)", raw_text, re.IGNORECASE)
                                if bg_ar: 
                                    bg = translate_to_english(bg_ar.group(1).strip())
                                    
                                fee_ar = re.search(r"(?:رسوم|Fees|Tender fees)\s*:\s*([^:\n\r]+)", raw_text, re.IGNORECASE)
                                if fee_ar: 
                                    fee = translate_to_english(fee_ar.group(1).strip())

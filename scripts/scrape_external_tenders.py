...
def beah_detail(page, url):
    try:
        ...
        m = re.search(r"Bid Submission Date\s+(.*?)(?:\s+Clarification End Date|\s+Tender Sells End Date|$)", text, re.I)
        close_date = normalize_date(m.group(1)) if m else ""
        return close_date
...
def parse_beah(page):
    ...
    for page_no in range(1, 11):
        ...
        for card in links:
            ...
            # Bidmate renders the real heading immediately before "Published";
            # the "More Details" link itself is NOT the tender title.
            title = ""
            before_published = re.split(r"\bPublished\b", text, maxsplit=1, flags=re.I)[0]
            before_published = clean(before_published)
            if before_published:
                title = before_published
...
def table_records(page, source, source_name, base_url):
    ...
    for tr in soup.find_all("tr"):
        ...
        records.append({
            ...
            "submission_close": "", "bid_open": "", "tender_url": detail,
            "keyword_matches": matches,
        })
    return records
...
def parse_jaggaer(page):
    records, visited = [], set()
    for url in [JAGGAER_HOME] + JAGGAER_CANDIDATES:
        try:
            page.goto(url, wait_until="domcontentloaded", timeout=60000)
        ...
        for detail in details[:200]:
            ...
            try:
                ...
                records.append({
                    ...
                    "purchase_start": "", "purchase_end": "", "submission_close": "",

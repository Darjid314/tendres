...
KEYWORDS = [
    ...
    "soc", "security operations center", "siem", "firewall", "endpoint security",
    "vulnerability assessment", "penetration testing", "vapt", "dlp",
    "zero trust", "network security", "threat intelligence", "edr", "xdr",
    "iso 27001", "ciso", "iam", "identity and access", "waf", "ddos", "pam",
    "cloud security", "security", "cyber",
    "الأمن السيبراني", "أمن المعلومات", "حماية البيانات", "الشبكات الأمنية", "أمن شبكات"
]
...
try:
    ...
    try:
        ...
        sheet = spreadsheet.add_worksheet(title=TARGET_TAB_NAME, rows="2000", cols="15")
        sheet.append_row([
...
def send_email_alert(new_tenders_list):
    ...
    msg['Subject'] = f"Alert: {len(new_tenders_list)} Cybersecurity Tenders Matched!"

    html = f"""
    <html>
    <body>
        <h2>Bhai, New Cybersecurity Tenders Matched!</h2>
...
async def main():
    async with async_playwright() as p:
        ...
        print("System Init: Connecting to Oman Tender Board Portal...")
        await page.goto(base_url, wait_until="networkidle", timeout=90000)
        
        content = await page.content()
        soup = BeautifulSoup(content, 'html.parser')
        total_pages = get_total_pages(soup)
        print(f"Total Pages detected: {total_pages}. Starting Scan...\n")
        ...
        for current_page in range(1, total_pages + 1):
            ...
            for index, row in enumerate(rows):
                ...
                if match_found:
                    print(f" TARGET MATCH FOUND: {tender_no} - {tender_title[:40]}...")
